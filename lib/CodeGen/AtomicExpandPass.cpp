...
/// operation. (That is, only the bits under the Mask may change)
///
/// Assumptions: Loaded may have any bits set; Inc may only have bits
/// under Mask set.
static Value *performMaskedAtomicOp(AtomicRMWInst::BinOp Op,
  ...
  case AtomicRMWInst::Add:
  case AtomicRMWInst::Sub: {
    // The result of add/sub must be masked, in case of overflow into
    // the higher bits.
  ...
  case AtomicRMWInst::Or:
  case AtomicRMWInst::Xor:
    // Logical ops won't affect any other bits, so can just be done
    // directly.
  ...
  case AtomicRMWInst::UMax:
  case AtomicRMWInst::UMin: {
    // Comparison ops operate on the full value, so truncate down
    // to the original size, and expand out again after doing the operation.
