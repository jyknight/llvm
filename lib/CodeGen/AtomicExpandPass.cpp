...
bool AtomicExpand::expandAtomicCmpXchg(AtomicCmpXchgInst *CI) {
  ...
  Function *F = BB->getParent();
  LLVMContext &Ctx = F->getContext();
  // If shouldInsertFencesForAtomic() returns true, then the target does not want
...
bool AtomicExpand::expandAtomicOpToLibcall(
  ...
  if (CASExpected) {
    ...
    AllocaCASExpected_i8 = Builder.CreateBitCast(AllocaCASExpected, Type::getInt8PtrTy(Ctx));
  ...
  if (ValueOperand) {
    if (UseSizedLibcall) {
      ...
      AllocaValue = AllocaBuilder.CreateAlloca(ValueOperand->getType(), nullptr, "");
      AllocaValue->setAlignment(AllocaAlignment);
      AllocaValue_i8 = Builder.CreateBitCast(AllocaValue, Type::getInt8PtrTy(Ctx));
  ...
  if (!CASExpected && HasResult && !UseSizedLibcall) {
    ...
    AllocaResult_i8 = Builder.CreateBitCast(AllocaResult, Type::getInt8PtrTy(Ctx));
