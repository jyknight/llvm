...
note, both sets of library functions (somewhat confusingly) share the names of
builtin functions defined by clang, but the library functions are entirely
separate from the builtins.

The first set of functions are named ``__atomic_*``. This set has been
"standardized" by GCC, and is described below. (See also `GCC's documentation
<https://gcc.gnu.org/wiki/Atomic/GCCMM/LIbrary>`_)

LLVM's AtomicExpandPass translates atomic operations on data sizes above
...
on function calls. For example, MIPS has a MIPS16 ISA, which is a shorter
instruction encoding. ARM has the Thumb ISA. In the former, and some versions of
the latter, the atomic instructions are not present. However, a function call
can be made to a MIPS32 or ARM function, to implement the operation.
...
ISelLowering code has set the corresponding operation to "Expand", and if it has
opted-into the availablity of those library functions via a call to
``initSyncLibcalls()``.
